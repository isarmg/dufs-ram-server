...
0.51.14 只允许全新、完整的当前实例状态。sarmg-upgrade 暂不支持该版本；不能将旧共享树和空状态库任意拼接，不能修改 metadata 或假设旧二进制能打开新状态。
...
Foundation 是编译期供应链输入，不是运行时共享服务。当前 Rust 固定正式 0.8.9 / `2908f7598571a47c1469bfa08a08c4776cc93049`，八个 Web 包使用同版 GitHub Release tarball 和锁文件 integrity，无相邻工作区依赖。Dufs 0.51.14 已按当前 Foundation 供应链完成独立源码树构建、浏览器回归和发布前核验。后续发行仍须执行同样门禁；依赖不可取得或身份不符时停止，不能复制共享类型、目标守卫或认证实现继续构建。
...
仓库另有 `.github/workflows/release-binary.yml`，只在推送 `v<version>` tag 后运行。轻量 `preflight` 先复核 tag、Cargo 版本和 workflow commit，一致后才调用完整正式包验收；候选核对任务消费正式验收上传的精确 artifact，并生成绑定当前版本与完整源码 SHA 的确定性发布输入。唯一的 `contents: write` job 不 checkout、不调用仓库脚本或执行下载的二进制，只消费并复核该不可变输入。结构化工作流策略还会逐 job 拒绝非发布任务的写权限，并锁定发布任务的 artifact 来源。

`.github/workflows/formal-release-e2e.yml` 在人工触发或版本 tag 的发布工作流复用时，使用 `ubuntu-26.04` 和临时 Ed25519 密钥调用未缩短的 `scripts/package-release.sh` 正式入口。它在含空格和 shell 元字符的隔离 clone 中建立精确本地 tag，实际经过完整质量门、vendor、release build、SBOM、checksum、签名和原子目录发布，再从外部复核四项制品、签名、公钥、包内 `SHA256SUMS` 以及二进制完整版本/SHA。该 job 只有 `contents: read`，不引用仓库或环境中的生产/自定义 secrets，只使用只读 GitHub token checkout；验收通过的四项候选以短期 artifact 精确交给后续只读核对任务，临时私钥仍随 runner 销毁，因此该 artifact 不是可分发的生产信任根。
...
bundle=/secure/releases/dufs-0.51.14-x86_64-unknown-linux-gnu-0123456789ab.release
