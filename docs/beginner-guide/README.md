...
文档只描述**当前工作区源码（Cargo 版本 0.51.13）**。产品运行时、页面、配置和教程不提供其他版本合同；未来稳定版本若需要处理非当前数据，只能使用 `sarmg-upgrade` 中精确绑定 source/target 并独立审核的迁移 adapter、fixture 与 CLI，不能据历史标签推断当前行为。
