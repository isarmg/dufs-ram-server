...
当前样例要求 nginx 1.25.1 或更高版本，编译时包含 HTTP SSL 与 HTTP/2 模块，并链接仍在上游或发行版安全维护期内的 OpenSSL；新部署优先选择 OpenSSL 3.5 LTS。每个 HTTPS `server` 块都只使用独立的 `http2 on;` 当前语法；项目只接受该版本范围和配置语法。切换流量前仍必须用目标主机上的 `nginx -t` 验证实际模块、证书路径和完整配置。
