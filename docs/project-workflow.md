...
```mermaid
flowchart LR
    SOURCE["仓库 web/login.html、index.html<br/>login.css、index.css、index.js、modules/ 下全部 .js、favicon"] --> COMPILE["include_str!/include_bytes!"]
...
flowchart TD
    ...
    L --> M["17. web/login.html / modules/ 下全部 .js<br/>登录、响应缓冲、分页、operation 协议和上传状态机"]
...
```mermaid
flowchart TD
    TEST["./scripts/check.sh（日常开发门）"] --> TOOLCHAIN["rustc --version / cargo --version<br/>确认固定的 1.98.0 工具链"]
    ...
    TEST --> STATIC["ESLint 浏览器安全规则 + TypeScript strict checkJs<br/>Bash 语法/可用时 ShellCheck + Markdown 链接<br/>生产解析器 YAML + systemd/nginx + 发布来源/树自测"]
